...
//            }
            ...
            if (rows.length > 3) {
                context = BCConvert.qj2bj(rows[3]);
            }
            Item item = new Item();
            item.setCode(code);
            // 枚数
//            context = context.replace("\"", "");
            if (StringUtils.isEmpty(context)) {
                ...
                int number = analysisNumber(context);
                if (context.contains("色セット")) {
                    item.setColor("Sx");
                    item.setNumber(Integer.parseInt(rows[2].replace("\"", "")));
                    item.setUnit("");
                } else {

                    String[] texts = context.split(":");
                    Preconditions.checkArgument(texts.length == 2, context + ":颜色异常");
//            Preconditions.checkArgument(texts[1].split("[^0-9]").length == 2, context + "枚数异常");
                    // 构建商品
        ...
        map.forEach((orderId, itemList) -> {
            ...
            items.forEach((newcode, newitemList) -> {
                ...
                for (int i = 0; i < newitemList.size(); i++) {
                    ...
                    builder.append(item.getColor()).append(item.getNumber()).append(item.getUnit());
    ...
    public static int analysisNumber(String text) {
        String regEx = "[^0-9]";
